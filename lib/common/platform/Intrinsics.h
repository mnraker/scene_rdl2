...
__forceinline void atomic_min_f32(volatile float *__restrict__ ptr, const float b)
{
  ...
  const int int_b = *(int*)const_cast<float*>(&b);
  while (1)
  {
    float a = *ptr;
    if (a <= b) break;
    const int int_a = *(int*)&a;
    const int result = atomic_cmpxchg(reinterpret_cast<int*>(const_cast<float*>(ptr)),int_a,int_b);
    // Intel: const int result = atomic_cmpxchg((int*)ptr,int_a,int_b);
...
__forceinline void atomic_min_i32(volatile int *__restrict__ ptr, const int b)
{
  while (1)
  {
    ...
    if (a <= b) break;
    const int int_a = *(int*)&a;
    const int result = atomic_cmpxchg(const_cast<int*>(ptr),int_a,b);
    // Intel: const int result = atomic_cmpxchg((int*)ptr,int_a,b);
    if (result == int_a) break;
  }
}
...
__forceinline void atomic_min_ui32(volatile unsigned int *__restrict__ ptr, const unsigned int b)
{
  while (1)
  {
    ...
    const unsigned int result = atomic_cmpxchg(reinterpret_cast<int*>(const_cast<unsigned int*>(ptr)),int_a,b);
    // Intel: const unsigned int result = atomic_cmpxchg((int*)ptr,int_a,b);
    if (result == int_a) break;
  }
}
