...
public:
    ...
    ValueContainerDeq(const void *addr, const size_t dataSize);
    ...
    ValueContainerDeq(const void *addr, const size_t dataSize, bool sizeCheck);

    // These are shallow copies and this is intentional.
    // Purpose of ValueContainerDeq is dequeueing data from original data memory and
    // we don't want to copy original data memory when we do copy/move
    // ValueContainerDeq is only define dequeue operation. You need to consider original
    // data memory management separately from ValueContainerDeq.
    ValueContainerDeq(const ValueContainerDeq &) noexcept = default;
    ValueContainerDeq(ValueContainerDeq &&) noexcept = default;

    virtual ~ValueContainerDeq() = default;
    ...
    inline void deqSceneObjectIndexable(StringVector &klassNameVec, StringVector &objNameVec);

    inline void deqAttributeType(ValueContainerUtil::ValueType &valueType);
    ...
    inline uintptr_t getCurrDataAddress() const { return (uintptr_t)mCurrPtr; } // for debug

    // make sure src ValueContainerDeq has same original encoded data buffer information
    inline bool isSameEncodedData(const ValueContainerDeq &src) const;
...
inline void
ValueContainerDeq::deqBool(bool &b)
{
...
inline void
ValueContainerDeq::deqChar(char &c)
{
...
inline void
ValueContainerDeq::deqUChar(unsigned char &uc)
{
...
inline void
ValueContainerDeq::deqUChar2(unsigned char &u0, unsigned char &u1)
{
    deqUChar(u0);
    deqUChar(u1);
}
...
inline void
ValueContainerDeq::deqUChar3(unsigned char &u0, unsigned char &u1, unsigned char &u2)
{
    deqUChar(u0);
    deqUChar(u1);
    deqUChar(u2);
...
inline void
ValueContainerDeq::deqUChar4(unsigned char &u0, unsigned char &u1,
...
inline void
ValueContainerDeq::deqInt(int &i)
{
...
inline void
ValueContainerDeq::deqUInt(unsigned int &ui)
{
...
inline void
ValueContainerDeq::deqLong(long &l)
{
...
inline void
ValueContainerDeq::deqULong(unsigned long &ul)
{
...
inline void
ValueContainerDeq::deqFloat12(float &f0, float &f1, float &f2,
                              float &f3, float &f4, float &f5,
                              float &f6, float &f7, float &f8,
                              float &f9, float &fa, float &fb)
{
...
inline void
ValueContainerDeq::deqString(std::string &str)
{
    unsigned long ul;
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, ul));
...
inline void
ValueContainerDeq::deqSceneObject(std::string &klassName, std::string &objName)
{
    unsigned long ul;
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, ul));
    size_t klassNameSize = static_cast<size_t>(ul);
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, ul));
    size_t objNameSize = static_cast<size_t>(ul);

    if (!klassNameSize && !objNameSize) {
        klassName.clear();
        objName.clear();
        VALUE_CONTAINER_DEQ_DEBUG_MSG("deqSceneObject() klass:>< obj:><\n");
    } else {
        klassName.resize(klassNameSize);
        objName.resize(objNameSize);
        const void *ptr = getDeqDataAddrUpdate(klassNameSize + objNameSize);
        ptr = loadCharN(ptr, &klassName[0], klassNameSize);
              loadCharN(ptr, &objName[0], objNameSize);
        VALUE_CONTAINER_DEQ_DEBUG_MSG("deqSceneObject() klass:>"
                                      << klassName << "< obj:>" << objName << "<\n");
    }
}

inline void
ValueContainerDeq::deqByteData(void *data, const size_t dataSize)
{
...
inline const void *
ValueContainerDeq::skipByteData(const size_t dataSize)
{
    return getDeqDataAddrUpdate(dataSize);
}

inline void
ValueContainerDeq::deqAlignPad()
{
    deqAlignPad(deqUShort());
}

inline void
ValueContainerDeq::deqAlignPad(const unsigned short padSize)
{
    skipByteData((size_t)padSize);
}

inline void
ValueContainerDeq::deqBoolVector(BoolVector &vec)
{
    unsigned long size;
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, size));
...
inline void
ValueContainerDeq::deqStringVector(StringVector &vec)
{
    unsigned long size;
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, size));
...
inline void
ValueContainerDeq::deqSceneObjectVector(StringVector &klassNameVec, StringVector &objNameVec)
{
    unsigned long size;
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, size));

    VALUE_CONTAINER_DEQ_DEBUG_MSG("deqSceneObjectVector() vecSize:>" << size << "<\n");
    klassNameVec.resize(size);
    objNameVec.resize(size);
    for (size_t i = 0; i < size; ++i) {
        unsigned long ul;
        updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, ul));
        size_t klassNameLen = static_cast<size_t>(ul);
        updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, ul));
        size_t objNameLen = static_cast<size_t>(ul);

        if (klassNameLen) {
            klassNameVec[i].resize(klassNameLen);
            const void *ptr = getDeqDataAddrUpdate(klassNameLen);
            loadCharN(ptr, &klassNameVec[i][0], klassNameLen);
        } else {
            klassNameVec[i].clear();
        }
        if (objNameLen) {
            objNameVec[i].resize(objNameLen);
            const void *ptr = getDeqDataAddrUpdate(objNameLen);
            loadCharN(ptr, &objNameVec[i][0], objNameLen);
        } else {
            objNameVec[i].clear();
        }
        VALUE_CONTAINER_DEQ_DEBUG_MSG("  deqSceneObjectVector() vec[" << i << "]"
                                      << " klass:>" << klassNameVec[i] << "<"
                                      << " obj:>" << objNameVec[i] << "<\n");
    }
}

inline void    
ValueContainerDeq::deqSceneObjectIndexable(StringVector &klassNameVec, StringVector &objNameVec)
{
    VALUE_CONTAINER_DEQ_DEBUG_MSG("deqSceneObjectIndexable() -> ");
    deqSceneObjectVector(klassNameVec, objNameVec);
}

inline void
ValueContainerDeq::deqAttributeType(ValueContainerUtil::ValueType &valueType)
{
    unsigned int ui;
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, ui));
    valueType = static_cast<ValueContainerUtil::ValueType>(ui);
    VALUE_CONTAINER_DEQ_DEBUG_MSG("deqAttributeType() valueType:>"
                                  << ValueContainerUtil::valueType2Str(valueType) << "<\n");
}

inline void
ValueContainerDeq::deqVLInt(int &i)
{
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, i));
...
inline void
ValueContainerDeq::deqVLUInt(unsigned int &ui)
{
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, ui));
...
inline void
ValueContainerDeq::deqVLLong(long &l)
{
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, l));
...
inline void
ValueContainerDeq::deqVLULong(unsigned long &ul)
{
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, ul));
...
inline void
ValueContainerDeq::deqVLIntVector(IntVector &vec)
{
    unsigned long size;
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, size));
...
inline void
ValueContainerDeq::deqVLLongVector(LongVector &vec)
{
    unsigned long size;
    updateCurrPtr(ValueContainerUtil::variableLengthDecoding(mCurrPtr, size));
...
inline bool    
ValueContainerDeq::isSameEncodedData(const ValueContainerDeq &src) const
{
...
inline const void *
ValueContainerDeq::loadChar(const void *ptr, char &c) const
{
...
inline const void *
ValueContainerDeq::loadUChar(const void *ptr, unsigned char &uc) const
{
...
inline const void *
ValueContainerDeq::loadCharN(const void *ptr, char *c, const size_t n) const
{
...
inline const void *
ValueContainerDeq::loadSizeT(const void *ptr, size_t &t) const
{
